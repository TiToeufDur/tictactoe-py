...
class Turn:
    ...
    def is_turn_valid(self,turn_oposite):
        for _ in toched[self.turn]:
            if _ in toched[turn_oposite]:
                print(f"{_}: Invalid(played)")
                toched[self.turn].pop()
                return False
            else:
                print(f"inspecter:{_}")
        return "Valide"
...
while True:
            ...
            if winner == None:
                for cell in cells:
                    if cell.is_clicked(pos):
                        ...
                        if toching in toched[turn.turn]:
                            print(f"{toched[turn.turn]}:Invalid(exists)")
                        else:
                            toched[turn.turn].append(toching) # Ajoute à la bonne liste selon le joueur courant
                            turn_oposite = turn.turn_oposite_turn()  #configure la variabe avec la classe
                            is_valid = turn.is_turn_valid(turn_oposite) # vérifie si le tour est valable en regardant si il existe deja dans les variables qui enregistre les tour passé
                            print(is_valid)
                            if is_valid:
                                print(f"joueur1:{toched[1]}  joueur2:{toched[2]}")
                                cell.symbol = "x" if turn.turn == 1 else "o"
                                for combo in win:
                                    if all(c in toched[turn.turn] for c in combo):
                                        winner = 'x' if turn.turn == 1 else 'o'
                                        winner_text = pygame.font.Font(None,30).render(f"{winner} wins!",True,"BLACK")
                                turn_value = turn.turn_counter()   # Incrémente le tour après avoir ajouté
                            # Check for tie (if all 9 cells are played and no winner)
                            if winner is None and len(toched[1]) + len(toched[2]) == 9:
                                winner = "tie"
                                winner_text = pygame.font.Font(None,30).render("Match nul!", True, "BLACK")
    ...
    if winner != None or winner == "tie" :
        ...
        screen.blit(recommencer_text,(0,screen.get_height() - recommencer_text.get_height()))
